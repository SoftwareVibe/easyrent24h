...
> - ✅ **STEP 6 — GATE CHIUSO** — admin Filament v5 su `/admin`: veicoli con listino (base+condizioni) via relation manager, località con hub/finestre orarie/endpoints-only **modificabili senza codice**, condizioni, extra, coupon, prenotazioni con stato "Blocco manuale", ordini con azione "Annulla e rimborsa" (libera il calendario), settings JSON. Test di rendering e accessi verdi.
> - ✅ **STEP 7 — GATE QUASI CHIUSO** — pannello `/vendor` separato per ruolo: il rappresentante vede solo i propri ordini/commissioni (testato); QR code SVG del link affiliato; coupon via URL sul frontend; **import storico**: 65 coupon WooCommerce reali + vendor legacy con pagamenti. Residuo: creare i vendor reali con credenziali (dato di business).
> - ✅ **GATE EXTRA "admin senza errori" (11/06)** — matrice di test su tutte le 12 risorse (render + salvataggi Livewire), fix errore intl (processi orfani), verifica Playwright; report in `report-gate-admin.md`.
> - ✅ **GATE EXTRA "lingua pannelli" (11/06)** — selettore EN/IT/ES nel menu utente di `/admin` e `/vendor`, persistito sul profilo utente (sopravvive a nuove sessioni), UI Filament tradotta; 9 test dedicati + verifica live.
> - 🔶 **STEP 8 — PREPARATO** — pagine Chi siamo/Contatti (form→API), WhatsApp, hreflang, mappa redirect 301 (`deploy/nginx-redirects.conf`), checklist cutover con rollback (`deploy/cutover-checklist.md`). Il **GATE 8 (go-live)** richiede: server di produzione, chiavi pagamento live, export sitemap Yoast per completare i redirect, switch DNS.
